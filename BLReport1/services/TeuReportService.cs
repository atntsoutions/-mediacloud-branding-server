...
namespace BLReport1
{
    public class TeuReportService : BL_Base
    {
        DataTable Dt_List = new DataTable();
        ExcelFile WB;
        ExcelWorksheet WS = null;
        List<TeuReport> mList = new List<TeuReport>();
        TeuReport mrow;
        int iRow = 0;
        int iCol = 0;
        string type = "";
        ...
        public IDictionary<string, object> List(Dictionary<string, object> SearchData)
        {
            ...
            try
            {
                ...
                //sql = " select cntr_pkid ,cntr_no,cntr_teu,cntr_csealno,cntr_asealno,   ";
                //sql += "   b.param_code as cntr_type_code, ";
                //sql += "   c.hbl_no as cntr_booking_no, ";
                //sql += "   c.hbl_bl_no as mbl_no,c.hbl_book_no as mbl_book_no,c.hbl_pol_etd as mbl_pol_etd,c.hbl_shipment_type as mbl_shipment_type,c.hbl_nature as mbl_nature ,";
                //sql += "   cntr_stuffed_at,cntr_stuffed_on,cntr_pcs,cntr_ntwt,cntr_grwt,cntr_cbm,cntr_clearing,";
                //sql += "   row_number() over(order by cntr_no) rn ";
                //sql += "   from containerm a  ";
                //sql += "   left join param b on a.cntr_type_id = b.param_pkid ";
                //sql += "   left join hblm c on a.cntr_booking_id = c.hbl_pkid ";
                //sql += "   where a.rec_company_code = '{COMPCODE}'";
                //sql += "   and a.rec_branch_code = '{BRCODE}'";
                //sql += "   and a.cntr_year = {YEARCODE} ";
                //if (type_date == "SOB")
                //    sql += "  and c.hbl_pol_etd between to_date('{FDATE}','DD-MON-YYYY') and to_date('{EDATE}','DD-MON-YYYY') ";
                //else
                //    sql += "  and to_char(a.rec_created_date,'DD-MON-YYYY') between to_date('{FDATE}','DD-MON-YYYY') and to_date('{EDATE}','DD-MON-YYYY') ";


                //if (type_date == "SOB")
                //    sql += " order by c.hbl_pol_etd";
                //else
                //    sql += " order by a.rec_created_date";

                if (rec_category == "TEU-EXPORT")
                {
                    sql = " select   SOB,cntr_pcs,cntr_ntwt,cntr_grwt,cntr_cbm,cntr_clearing,cntr_csealno,cntr_asealno,cntr_booking_no,cntr_stuffed_on";
                    sql += " ,cntr_no,mbl_no,mbl_book_no,branch";
                    sql += " ,replace(case when instr(ctype.param_name,'45') > 0 then '40HC' else to_char(ctype.param_name) end,' ','') as cntr_type_code";
                    sql += " ,Cntr_teu,cntr_shipment_type";
                    ...
                    sql += " ,decode(consignee_tot,1,nvl(nomination,cons.cust_nomination), 'VARIOUS') as Nomination";
                   // sql += " ,decode(Nom_tot,1,Nomination, 'VARIOUS') 	  as Nomination";
                    ...
                    sql += " ,nvl(sman3.param_name,nvl(sman2.param_name, sman.param_name)) as salesman";
                    ...
                    sql += " h.rec_branch_code,";
                    sql += " max(mb.hbl_nature) as container_Nature,";
                    sql += " max(mb.hbl_shipment_type) as cntr_shipment_type,";
                    sql += " max(mb.hbl_bl_no) as mbl_no,";
                    sql += " max(mb.hbl_no) as cntr_booking_no,";

                    sql += " max(mb.hbl_book_no) as mbl_book_no,";

                    sql += " max(mb.hbl_pol_etd) as Sob,";
                    ...
                    sql += " max(h.hbl_carrier_id)     as Liner_Id,    count(distinct h.hbl_carrier_id)     as Liner_Tot,";
                    sql += " max(h.hbl_Agent_id)     as Agent_Id,    count(distinct h.hbl_Agent_id)     as Agent_Tot,";
                    ...
                    sql += " inner  join hblm        h  on (j.jobs_hbl_id = h.hbl_pkid)";
                    sql += " inner  join hblm        mb on (h.hbl_mbl_id = mb.hbl_pkid)";
                    ...
                    if (type_date == "SOB")
                        sql += "  and mb.hbl_pol_etd between to_date('{FDATE}','DD-MON-YYYY') and to_date('{EDATE}','DD-MON-YYYY') ";
                    else
                        sql += "  and j.job_date between to_date('{FDATE}','DD-MON-YYYY') and to_date('{EDATE}','DD-MON-YYYY') ";
                    //sql += "  and to_char(d.rec_created_date,'DD-MON-YYYY') between to_date('{FDATE}','DD-MON-YYYY') and to_date('{EDATE}','DD-MON-YYYY') ";
                    sql += " group by d.rec_branch_code,cntr_pkid,h.rec_branch_code";
                    sql += " )  a";
                    ...
                    foreach (DataRow Dr in Dt_List.Rows)
                    {
                        ...
                        if (Dr["cntr_shipment_type"].ToString() == "LCL" || Dr["job_type"].ToString() == "CLEARING")
                            mrow.cntr_teu = 0;
                        mrow.hbl_exp_name = Dr["shipper_name"].ToString();
                        mrow.hbl_imp_name = Dr["consignee_name"].ToString();
                        mrow.hbl_nomination = Dr["Nomination"].ToString();
                        ...
                        mrow.mbl_shipment_type = Dr["cntr_shipment_type"].ToString();
                        mrow.mbl_nature = Dr["container_Nature"].ToString();
                ...
                if (rec_category == "TEU-IMPORT")
                {
                    sql = " select   sob,cntr_pcs,cntr_ntwt,cntr_grwt,cntr_cbm,cntr_no,mbl_no,branch";
                    sql += "  ,replace(case when instr(ctype.param_name,'45') > 0 then '40HC' else to_char(ctype.param_name) end,' ','') as cntr_type_code";
                    sql += "  ,case when instr(ctype.param_name,'20') > 0 then 1 else";
                    sql += "   case when instr(ctype.param_name,'40') > 0 or instr(ctype.param_name,'45') > 0 then 2 else 0  end end as Cntr_teu";
                    sql += "  ,Nomination";
                    sql += "  ,Agent.cust_Name  as Agent_name";
                    sql += "  ,Agent.cust_Code  as Agent_code";
                    sql += "  ,Liner.param_Name   as Liner_name";
                    sql += "  ,Liner.param_Code   as Liner_code";
                    ...
                    sql += "  ,decode(consignee_tot,1,cons.cust_code, 'VARIOUS') as consignee_code";
                    sql += "  ,a.cntr_pkid";
                    sql += "  ,consignee_Id, Consignee_Tot,Liner_Id,Agent_Id,branch_code";
                    sql += "  ,Exporter_Id, Exporter_Tot";
                    sql += "  ,Country_Id,Country_Tot";
                    sql += "  ,nvl(sman2.param_name, sman.param_name) as salesman";
                    sql += "  ,a.rec_branch_code as br_location";
                    sql += "  ,job_type ";
                    sql += "   from ( ";
                    sql += "   select cntr_pkid,";
                    sql += "  d.rec_branch_code as branch,";
                    sql += "  h.rec_branch_code as rec_branch_code,";
                    sql += "  max(mb.hbl_bl_no) as mbl_no,";
                    sql += "  max(mb.hbl_pol_etd) as sob,";
                    sql += "  max(mb.rec_branch_code) as branch_code,  ";
                    sql += "  max(mb.hbl_carrier_id)     as Liner_Id,  ";
                    sql += "  max(mb.hbl_Agent_id)     as Agent_Id,    ";
                    sql += "  max(mb.hbl_nomination)  as Nomination,   ";
                  
                    sql += "  max(mb.hbl_pod_id)      as Pod_Id,     ";
                    sql += "  max(mb.hbl_pofd_id)      as Pofd_Id,   ";
                    sql += "  max(mb.hbl_jobtype)  as job_type,";
                    sql += "  max(h.hbl_exp_id)  as Exporter_Id ,count(distinct h.hbl_exp_id)  as Exporter_Tot,";
                    sql += "  max(h.hbl_imp_id) as Consignee_Id,count(distinct h.hbl_imp_id) as Consignee_Tot,";
                    sql += "  max(h.hbl_pol_id) as Pol_Id, count(distinct h.hbl_pol_id) as Pol_Tot,";
                    sql += "  max(h.hbl_origin_country_id)  as Country_Id,  count(distinct h.hbl_origin_country_id)  as Country_Tot ";
                    sql += "  from  impcontainerm d";
                    sql += "  inner join hblm  mb on (d.cntr_parent_id = mb.hbl_pkid)";
                    sql += "  inner join hblm  h  on (mb.hbl_pkid = h.hbl_mbl_id)";
                    sql += "  where mb.hbl_jobtype in ('FORWARDING','BOTH')";
                    ...
                    if (consginee_id.Length > 0)
                        sql += " and h.hbl_exp_id = '" + consginee_id + "' ";

                    if (shipper_id.Length > 0)
                        sql += " and h.hbl_imp_id = '" + shipper_id + "' ";

                    if (agent_id.Length > 0)
                        sql += " and mb.hbl_Agent_id = '" + agent_id + "' ";
                    if (carrier_id.Length > 0)
                        sql += " and mb.hbl_carrier_id = '" + carrier_id + "'";
                    if (pol_id.Length > 0)
                        sql += " and h.hbl_pol_id = '" + pol_id + "'";
                    if (pod_id.Length > 0)
                        sql += " and mb.hbl_pod_id = '" + pod_id + "'";

                    if (type_date == "SOB")
                        sql += "  and mb.hbl_pol_etd between to_date('{FDATE}','DD-MON-YYYY') and to_date('{EDATE}','DD-MON-YYYY') ";
                    else
                        sql += "  and mb.hbl_date between to_date('{FDATE}','DD-MON-YYYY') and to_date('{EDATE}','DD-MON-YYYY') ";


                    sql += "  group by d.rec_branch_code,cntr_pkid,h.rec_branch_code";

                    sql += "  ) a";
                    sql += "  left join impcontainerm cont on (a.cntr_pkid = cont.cntr_pkid)";
                    sql += "  left join customerm cons on (a.consignee_id = cons.cust_pkid)";
                    sql += "  left join param Liner on (a.Liner_id = Liner.param_pkid)";
                    sql += "  left join customerm Agent on (a.Agent_id = Agent.cust_pkid)";
                    sql += "  left join customerm 	 shipper on (a.exporter_id =shipper.cust_pkid)";
        ...
        private void PrintTeuReport()
        {
            ...
            try
            {
                ...
                if(all)
                {
                    ...
                    WS.Columns[25].Style.NumberFormat = "#0.00";
                    WS.Columns[26].Style.NumberFormat = "#0.000";
                    WS.Columns[27].Style.NumberFormat = "#0.000";
                    WS.Columns[28].Style.NumberFormat = "#0.000";
                    WS.Columns[29].Style.NumberFormat = "#0.00";
                }
                else
                {
                    WS.Columns[24].Style.NumberFormat = "#0.00";
                ...
                iRow++;
                iRow++;
                Lib.WriteData(WS, iRow, 1, "TEU REPORT", _Color, true, "", "L", "", 15, false, 325, "", true);
                ...
                Lib.WriteData(WS, iRow, iCol++, "SHIPMENT TYPE", _Color, true, "BT", "L", "", _Size, false, 325, "", true);
                ...
                List<TeuReport> MList = new List<TeuReport>();
                foreach (TeuReport Rec in mList)
                {
                    iRow++;
                    iCol = 1;
                    if (Rec.row_type == "DETAIL")
                    {
                        ...
                        Lib.WriteData(WS, iRow, iCol++, Rec.cntr_teu, _Color, false, "", "R", "", _Size, false, 325, "#,0.00;(#,0.00);#", false);
                        Lib.WriteData(WS, iRow, iCol++, Rec.cntr_pcs, _Color, false, "", "R", "", _Size, false, 325, "#,0.00;(#,0.00);#", false);
                        Lib.WriteData(WS, iRow, iCol++, Rec.cntr_ntwt, _Color, false, "", "R", "", _Size, false, 325, "#,0.00;(#,0.00);#", false);
                        Lib.WriteData(WS, iRow, iCol++, Rec.cntr_grwt, _Color, false, "", "R", "", _Size, false, 325, "#,0.00;(#,0.00);#", false);
                        Lib.WriteData(WS, iRow, iCol++, Rec.cntr_cbm, _Color, false, "", "R", "", _Size, false, 325, "#,0.00;(#,0.00);#", false);
                    }
                    if (Rec.row_type == "TOTAL")
                    {
                        ...
                        Lib.WriteData(WS, iRow, iCol++, "", _Color, false, "BT", "C", "", _Size, false, 325, "", true);
                        Lib.WriteData(WS, iRow, iCol++, "", _Color, false, "BT", "C", "", _Size, false, 325, "", true);
                        Lib.WriteData(WS, iRow, iCol++, Rec.cntr_teu, _Color, true, "BT", "R", "", _Size, false, 325, "", true);
                        Lib.WriteData(WS, iRow, iCol++, "", _Color, true, "BT", "R", "", _Size, false, 325, "", true);
                        Lib.WriteData(WS, iRow, iCol++, Rec.cntr_ntwt, _Color, true, "BT", "R", "", _Size, false, 325, "", true);
                        Lib.WriteData(WS, iRow, iCol++, Rec.cntr_grwt, _Color, true, "BT", "R", "", _Size, false, 325, "", true);
                        Lib.WriteData(WS, iRow, iCol++, Rec.cntr_cbm, _Color, true, "BT", "R", "", _Size, false, 325, "", true);
        ...
        private void PrintImportTeuReport()
        {
            string str = "";
            string COMPNAME = "";
            string COMPADD1 = "";
            ...
            try
            {
                ...
                File_Display_Name = "ImportTeu.xls";
                File_Name = Lib.GetFileName(report_folder, PKID, File_Display_Name);
                ...
                {
                    WS.Columns[23].Style.NumberFormat = "#0.00";
                    WS.Columns[24].Style.NumberFormat = "#0.00";
                    WS.Columns[25].Style.NumberFormat = "#0.000";
                    WS.Columns[26].Style.NumberFormat = "#0.000";
                    WS.Columns[27].Style.NumberFormat = "#0.000";
                ...
                iRow++;
                iRow++;
                Lib.WriteData(WS, iRow, 1, "IMPORT TEU REPORT", _Color, true, "", "L", "", 15, false, 325, "", true);
                // Lib.WriteMergeCell(WS, iRow, 1, 9, 1,"TEU REPORT", "Calibri", 15, true, Color.Black, "C", "C", "", "");
                iRow++;
                iRow++;
                _Size = 10;
                ...
                foreach (TeuReport Rec in mList)
                {
                    ...
                    if (Rec.row_type == "TOTAL")
                    {
                        ...
                        Lib.WriteData(WS, iRow, iCol++, "", _Color, false, "BT", "C", "", _Size, false, 325, "", true);
                        Lib.WriteData(WS, iRow, iCol++, "", _Color, false, "BT", "C", "", _Size, false, 325, "", true);
                        Lib.WriteData(WS, iRow, iCol++, Rec.cntr_teu, _Color, true, "BT", "R", "", _Size, false, 325, "#,0.00;(#,0.00);#", false);
                        Lib.WriteData(WS, iRow, iCol++, "", _Color, true, "BT", "R", "", _Size, false, 325, "", true);
