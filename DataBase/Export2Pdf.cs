...
namespace DataBase
{
    public class Export2Pdf
    {
        ...
        private void AddImage(string str)
        {
            ...
            int PdfY = int.Parse(sData[6].ToString());
            DrawImage(ImageName, Left, Top, Height, Width, PdfY);

        }
        private void DrawImage(string ImgName, int x, int y, int height, int width, int Pdy)
        {
            try
            {
                if (System.IO.File.Exists(ImgName))
                {
                    ...
                    if (height > 0 && width > 0)
                        gfx.DrawImage(newImage, x, y, width, height);
                    else
