...
namespace BLPim
{
    public class SpotService : BL_Base
    {
        ...
        public Dictionary<string, object> GetRecord(Dictionary<string, object> SearchData)
        {
            ...
            try
            {
                ...
                foreach (DataRow Dr in Dt_Rec.Rows)
                {
                    ...
                    mRow.spot_store_view_file_uploaded = false;
                    if (Dr["spot_store_view"].ToString().Length > 0)
                        mRow.spot_store_view_file_uploaded = true;

                    mRow.spot_installation_view_file_uploaded = false;
                    if (Dr["spot_installation_view"].ToString().Length > 0)
                        mRow.spot_installation_view_file_uploaded = true;
        ...
        public Dictionary<string, object> Save(pim_spot Record)
        {
            ...
            try
            {
                ...
                if (Record.spot_executive_name.Trim().Length <= 0)
                    Lib.AddError(ref ErrorMessage, "Executive Name Cannot Be Empty");

                if (Record.spot_store_contact_name.Trim().Length <= 0)
                    Lib.AddError(ref ErrorMessage, "Contact Name Cannot Be Empty");

                if (Record.spot_store_contact_tel.Trim().Length <= 0)
                    Lib.AddError(ref ErrorMessage, "Contact Tel Cannot Be Empty");
                ...
                Rec.InsertString("spot_store_view", Record.spot_store_view);
                Rec.InsertString("spot_installation_view", Record.spot_installation_view);
        ...
        public Dictionary<string, object> DeleteDet(Dictionary<string, object> SearchData)
        {
            Boolean bRet = false;
            Dictionary<string, object> RetData = new Dictionary<string, object>();
            ...
            DataTable dt_test = new DataTable();
            sql = "select spotd_pkid from pim_spotd where spotd_parent_id = '" + parentid + "' and spotd_pkid != '" + pkid + "' order by spotd_slno";
            dt_test = Con_Oracle.ExecuteQuery(sql);

            try
            {
                Con_Oracle.BeginTransaction();
                sql = "delete from pim_spotd where spotd_pkid = '" + pkid + "'";
                Con_Oracle.ExecuteNonQuery(sql);
                foreach (DataRow Dr in dt_test.Rows)
                {
                    islno++;
                    sql = "update pim_spotd set spotd_slno = " + islno.ToString() + " where spotd_pkid = '" + Dr["spotd_pkid"] + "'";
                    Con_Oracle.ExecuteNonQuery(sql);
                }
                Con_Oracle.CommitTransaction();
                Con_Oracle.CloseConnection();
