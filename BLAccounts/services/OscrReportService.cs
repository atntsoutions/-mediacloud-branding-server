...
{
    public class OscrReportService : BL_Base
    {
        ...
        public IDictionary<string, object> List(Dictionary<string, object> SearchData)
        {
          ...
          //  string sman_name = "";
            ...
            showzero = (Boolean)SearchData["showzero"];
            ...
            {
                SQL = "";
                SQL += " select branch_code,jv_pkid,jv_acc_id,jvh_cc_id,jvh_vrno,jvh_docno,jvh_date, jvh_org_invdt, jvh_reference_date, xref_dr_jv_date,cust_code, cust_name, jv_debit, jv_credit,adv, balance, invtype,cust_crdays, cust_crlimit, os_days,os_days - cust_crdays as overdue, ";
                SQL += " jv_od_type, jv_od_remarks ";
                SQL += " from (";
                ...
                SQL += "  0 as adv, jvh_org_invdt, jvh_reference_date, max(xref_dr_jv_date) as xref_dr_jv_date,  max(jv_od_type) as jv_od_type, max(jv_od_remarks) as jv_od_remarks,";

                if (IsTillDate)
                    SQL += " trunc(sysdate -  nvl(nvl(jvh_org_invdt,jvh_reference_date),jvh_date)  ,0)  as os_days   ";
                else
                    SQL += " trunc( to_date('{EDATE}','DD-MON-YYYY') -  nvl(nvl(jvh_org_invdt,jvh_reference_date),jvh_date)  ,0)  as os_days   ";
                ...
                SQL += " group by h.rec_branch_code,jv_pkid,jv_acc_id,jvh_cc_id,jvh_date,jvh_org_invdt, jvh_reference_date,jvh_vrno,jvh_docno,jv_credit  ";
                ...
                if (IsOverDue == false)
                {
                    ...
                    SQL += " nvl(sum(xref_amt),0) - jv_debit   as adv, jvh_org_invdt, jvh_reference_date,max(xref_dr_jv_date) as xref_dr_jv_date, max(jv_od_type) as jv_od_type, max(jv_od_remarks) as jv_od_remarks, ";
                    SQL += " 0 as os_days ";
                    SQL += " from ledgerh h ";
                    SQL += " inner join ledgert L on (h.jvh_pkid = L.jv_parent_id )";
                    SQL += " inner join  Acctm a on (L.jv_acc_id = A.acc_pkid )   ";
                    SQL += " inner join acgroupm g on a.acc_group_id = g.acgrp_pkid and g.rec_company_code ='{COMPCODE}' and acgrp_name  = 'SUNDRY CREDITORS'";
                    SQL += " left join ledgerxref X on (L.jv_pkid=X.xref_dr_jv_id {CRSQL} )";
                    SQL += " where ";
                    ...
                    SQL += " and L.jv_debit >0 and h.REC_DELETED = 'N'  and acc_against_invoice ='C' and jvh_type not in('OP','OB','OC') ";
                    SQL += " group by h.rec_branch_code,jv_pkid,jv_acc_id,jvh_date, jvh_org_invdt, jvh_reference_date,jvh_vrno,jvh_docno,jv_debit";
                    if (showzero == false)
              ...
              //  SQL += " left join custdet cd on a.jv_acc_id = cd.det_cust_id and a.branch_code =  cd.det_branch_code ";
                //SQL += " left join param sman on hbl_salesman_id = sman.param_pkid";
               ...
               // SQL += " left join param sman2 on cd.det_sman_id = sman2.param_pkid";
                ...
                foreach (DataRow Dr in Dt_List.Rows)
                {
                   ...
                   // mrow.sman_name = sman_name;
                    ...
                    if (!Dr["xref_dr_jv_date"].Equals(DBNull.Value))
                    {
                        mrow.jv_paid_date = Lib.DatetoStringDisplayformat((DateTime)(Dr["xref_dr_jv_date"]));
                    }
                    if (!Dr["jvh_org_invdt"].Equals(DBNull.Value))
                    {
                        jv_date = (DateTime)(Dr["jvh_org_invdt"]);
                        mrow.jv_org_invdt = Lib.DatetoStringDisplayformat(jv_date);
                    }
                    else if (!Dr["jvh_reference_date"].Equals(DBNull.Value))
                    {
                        jv_date = (DateTime)(Dr["jvh_reference_date"]);
                        mrow.jv_org_invdt = Lib.DatetoStringDisplayformat(jv_date);
                    }
...

        {
         ...
         //   WS.Columns[10].Style.NumberFormat = "#,0.00";
            ...
            sTitle = "CREDITORS OUTSTANDING REPORT AS ON " + Lib.getFrontEndDate(to_date);
          ...
          //  Lib.WriteData(WS, iRow, iCol++, "SALESMAN", _Color, true, _Border, "L", "", _Size, false, 325, "", true);
            ...
            Lib.WriteData(WS, iRow, iCol++, "INV-DATE", _Color, true, _Border, "L", "", _Size, false, 325, "", true);
            Lib.WriteData(WS, iRow, iCol++, "PAID-DATE", _Color, true, _Border, "L", "", _Size, false, 325, "", true);
            ...
            foreach (LedgerReport Dr in mList)
            {
               ...
               // Lib.WriteData(WS, iRow, iCol++, nvl(Dr.sman_name, ""), _Color, _Bold, _Border, "L", "", _Size, false, 325, "", true);
                ...
                Lib.WriteData(WS, iRow, iCol++, Lib.nvlDate(Dr.jv_org_invdt, "", Lib.FRONT_END_DATE_DISPLAY_FORMAT), _Color, _Bold, _Border, "L", "", _Size, false, 325, "dd/MM/yyyy", true);
                Lib.WriteData(WS, iRow, iCol++, Lib.nvlDate(Dr.jv_paid_date, "", Lib.FRONT_END_DATE_DISPLAY_FORMAT), _Color, _Bold, _Border, "L", "", _Size, false, 325, "dd/MM/yyyy", true);
